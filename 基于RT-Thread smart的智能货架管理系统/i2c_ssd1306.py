...
def oled_init():
    ...
    send_command(0xA1)  # 修改：Set segment re-map (A0 for normal, A1 for reverse)，这里设置为正常映射
    send_command(0xC0)  # 修改：Set COM output scan direction (C0 for normal, C8 for reverse)，这里设置为正常扫描方向
